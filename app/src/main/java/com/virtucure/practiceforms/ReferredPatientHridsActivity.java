 ...
 */
public class ReferredPatientHridsActivity extends AppCompatActivity {

    private Toolbar toolbar;
    private RecyclerView recyclerView;
    private BroadcastReceiver logoutReceiver;
    private LinearLayoutManager linearLayoutManager;
    private List<ReferredPatientHridDTO> referredHridsList;
    private ReferredPatientsHridsAdapter adapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        getHridsList(projectType, brandorBranchName);
    ...
    private void getHridsList(final String projectType, final String brandorBranchName) {
                ...
                if (!result.isEmpty() && result.get("result") != null) {
                    try {
                        ...
                        if("success".equalsIgnoreCase(jsonObject.getString("status"))) {
                            referredHridsList = new Gson().fromJson(jsonObject.getString("result"), new TypeToken<List<ReferredPatientHridDTO>>(){}.getType());
                            adapter = new ReferredPatientsHridsAdapter(referredHridsList, new OnRecyclerItemClickListener() {
                                @Override
                                public void onRecyclerItemClick(Object obj) {
                                    ReferredPatientHridDTO item = (ReferredPatientHridDTO) obj;
                                    Intent intent = new Intent(getApplicationContext(), ReferredPatientCaseRecordsActivity.class);
                                    intent.putExtra("projectType", projectType);
                                    intent.putExtra("brandorBranchName", brandorBranchName);
                                    intent.putExtra("hrid", item.getHrid());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_logout) {
            ...
            AsyncWorkerNetwork worker = new AsyncWorkerNetwork(
                    ReferredPatientHridsActivity.this, new ActionCallback() {
                public void onCallback(Map<String, String> result) {
