 ...
 */
public class ReferredPatientBranchesActivity extends AppCompatActivity {

    private Toolbar toolbar;
    private RecyclerView recyclerView;
    private BroadcastReceiver logoutReceiver;
    private LinearLayoutManager linearLayoutManager;
    private List<ReferredPatientsDTO> referredBranchesList;
    private ReferredPatientsBranchesAdapter adapter;
    ...
    private void getBranchesList(final String projectType) {
        ...
        params.put("requiredType", "brandorBranchName");
                ...
                if (!result.isEmpty() && result.get("result") != null) {
                    try {
                        ...
                        if("success".equalsIgnoreCase(jsonObject.getString("status"))) {
                            referredBranchesList = new Gson().fromJson(jsonObject.getString("result"), new TypeToken<List<ReferredPatientsDTO>>(){}.getType());
                            adapter = new ReferredPatientsBranchesAdapter(referredBranchesList, new OnRecyclerItemClickListener() {
                                @Override
                                public void onRecyclerItemClick(Object obj) {
                                    ReferredPatientsDTO item = (ReferredPatientsDTO) obj;
                                    Intent intent = new Intent(getApplicationContext(), ReferredPatientHridsActivity.class);
                                    intent.putExtra("projectType", projectType);
                                    intent.putExtra("brandorBranchName", item.getBrandorBranchName());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_logout) {
            ...
            AsyncWorkerNetwork worker = new AsyncWorkerNetwork(
                    ReferredPatientBranchesActivity.this, new ActionCallback() {
                public void onCallback(Map<String, String> result) {
